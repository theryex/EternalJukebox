...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # Since it's behind your tunnel, * is easiest
...
def run_full_process(track_id, meta):
    # Initialize a variable to track the last message sent
    last_msg = ""

    def update_log(msg, prog=None):
        nonlocal last_msg
        
        # If the message is the same as the last one, skip the update and the sleep
        if msg == last_msg:
            return
        ...
        # Only sleep for new, unique messages
        time.sleep(0.3)
